...
public enum RetCode {
	...
	/**
	 * 请求倍拒绝, 因为状态原因, leaving的分区不允许send消息, joining的分区不允许除send外的请求
	 */
	REQUEST_REFUSE("1007"),
	/**
	 ...
	 * 和 {@link RetCode#INTERNAL_SERVER_ERROR} 不同的是客户端之后的重试都是有意义的。<br/>
	 ...
	 */
	private String code;
	private static final Map<String, RetCode> CACHE = Maps.newHashMap();
	...
	/**
	 * @param name
	 * @return {@link RetCode} 对象实例
	 */
	public static RetCode valueOf(Object name) {
	 ...
	 * @return {@link RetCode}对象实例
	 */
	public static RetCode valueOfCode(String code) {
		synchronized (CACHE) {
			if (CACHE.isEmpty()) {
				final RetCode[] values = values();
				for (final RetCode value : values) {
		...
		throw new IllegalArgumentException("No enum const class " + RetCode.class + ".(code="
				+ code + ")");
	}
	...
	public static void main(String[] args) {
		System.out.println(RetCode.valueOfCode("0"));
		System.out.println(RetCode.valueOfCode("010"));
	}
}
