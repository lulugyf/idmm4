...
public class BLEActor extends AbstractActor {
    ...
    private Timer timer_m; //= metrics.timer(name(RequestHandler.class, "responses"));
    private Timer timer_rm;
    ...
    private void onReceive(RefMsg s){
        ...
        }else if("metrics".equals(s.name)){
            metrics = (MetricRegistry)(s.obj);
//            timer_m = metrics.timer(name(BLEActor.class, "timer_m"));
            timer_rm = metrics.timer(name(BLEActor.class, "timer_rm"));
        }
    }
...
//        final Timer.Context context = timer_m.time();
//        try {
            int seq = seq_seed++;
            switch (s.fm.getType()) {
                case BRK_PULL: {
...
//        final Timer.Context context = timer_rm.time();
//        try {
...
//        log.debug("receive packet seq {}", seq);
            ...
            timer_rm.update(System.currentTimeMillis()-s.create_time,
